...
export const ProtectedRoute = ({ onlyUnAuth = false, children }: ProtectedRouteProps): JSX.Element => {
    ...
    if (!user && !onlyUnAuth ) { // если пользователя в хранилище нет, то делаем редирект
        return <Navigate replace to='/login' state={{ from: location }} />; 
    }
    
    if (user && onlyUnAuth ) { // если пользователь есть в хранилище
...
export const OnlyUnAuth = ({ children }: { children: React.ReactElement }): JSX.Element => (
